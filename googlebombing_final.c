...
static void ensure_counts_capacity(int **counts, int *capacity, int needed_index) {
    if (needed_index < *capacity) return;
    ...
    for (int i = old_capacity; i < new_capacity; i++) new_counts[i] = 0;
    *counts = new_counts;
...
static Graph read_graph(const char *filename) {
    ...
    if (max_id > capacity) ensure_counts_capacity(&row_counts, &capacity, max_id);
...
static void multiply_base(const Graph *g, const double *pi, double alpha, double *next) {
    double dangling = 0.0;
    for (int i = 0; i < g->n; i++) next[i] = 0.0;
    ...
    for (int i = 0; i < g->n; i++) next[i] += uniform;
}
...
static PageRankRun pagerank_base(const Graph *g, double alpha, double *pr) {
    ...
    for (int i = 0; i < g->n; i++) cur[i] = 1.0 / g->n;
...
static void multiply_attack(const Graph *g, const double *pi, double alpha,
    ...
    for (int i = 0; i < total_n; i++) next[i] = 0.0;
    ...
    for (int a = 0; a < k; a++) attack_mass += pi[first_attacker + a];
    ...
    for (int i = 0; i < total_n; i++) next[i] += uniform;
}

/*
 * Initialisation "chaude": on suppose que la cible a deja un PageRank de base
 * et on garde la masse repartie sur les anciens noeuds proportionnellement,
 * en allouant la part 1/total_n aux nouveaux attaquants. Cela accelere la
 * convergence par rapport a un demarrage uniforme strict.
 */
static void warm_init(const double *pr_base, int base_n, int k, double *cur) {
    int total_n = base_n + k;
    double scale = (double)base_n / (double)total_n;
    for (int i = 0; i < base_n; i++) cur[i] = pr_base[i] * scale;
    double share = 1.0 / (double)total_n;
    for (int a = 0; a < k; a++) cur[base_n + a] = share;
}
...
static PageRankRun pagerank_attack(const Graph *g, double alpha, AttackType type,
    ...
    if (pr_base) {
        ...
        for (int i = 0; i < total_n; i++) cur[i] = 1.0 / total_n;
    }
...
static const char *target_name(int i) {
    switch (i) {
        case TARGET_FORTE:   return "forte";
        case TARGET_MOYENNE: return "moyenne";
        case TARGET_FAIBLE:  return "faible";
        default: return "inconnu";
    }
}
...
static void run_simulation(const Graph *g, double alpha, int k_max,
    ...
    if (opts->professor_k) {
        ...
        for (int i = 0; i < nb_sizes; i++) {
            ...
            last_k = k;
            if (opts->verbose) printf("  k=%d termine\n", k);
        }
        if (last_k != k_max) {
...
int main(int argc, char **argv) {
    ...
    if (opts.verbose) {
        ...
        printf("alphas   :");
        for (int i = 0; i < opts.n_alphas; i++) printf(" %.3f", opts.alphas[i]);
        printf("\nSortie   : %s\n", csv_path);
    }

    Graph g = read_graph(filename);
    if (opts.verbose) printf("Graphe charge: n=%d, m=%d\n", g.n, g.m);

    if (k_max == 0) {
        if (opts.verbose) printf("Mode verification: lecture OK, pas de simulation.\n");
        free_graph(&g);
        return EXIT_SUCCESS;
    }
    ...
    fclose(csv);
    if (opts.verbose) printf("Resultats exportes dans %s\n", csv_path);
